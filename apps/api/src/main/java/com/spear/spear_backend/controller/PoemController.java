...
public class PoemController {
    ...
    public Page<Poem> searchPoems(
        ...
        return poemService.searchPoems(
            title,
            authorId,
            tags,
            content,
    ...
    public ResponseEntity<Poem> createPoem(
        @RequestBody CreatePoemRequest request
    ) {
        Poem poem = new Poem();
    ...
    @PutMapping("/{id}/like")
    public ResponseEntity<Poem> likePoem(@PathVariable String id) {
        Poem updatedPoem = poemService.likePoem(id);
        if (updatedPoem == null) {
